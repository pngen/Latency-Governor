// 05_ttft_protection.cpp
// A prefill observation that exceeds the ttf target makes govern_prefill protect
// time-to-first-token.
//
...
int main() {
    ...
    RequestDescriptor d = make_req(1, 1, 1, SloClass::REALTIME);
    d.contract.e2e_target = ms(500);
    d.contract.ttf_target = ms(20);
    d.contract.prefill_target = ms(150);
    const AdmitResult a = gov.admit(d);

    // Observed prefill chunk took longer than the ttf budget (25ms > 20ms).
    gov.observe(obs_for(a, ObservationType::PREFILL_CHUNK_COMPLETED, Phase::PREFILL, ms(25)));

    const PrefillGovernance pg = gov.govern_prefill(a.request_id);
    std::printf("ttf_target_ns              = %lld\n", (long long)ns_count(d.contract.ttf_target));
    std::printf("observed prefill chunk_ns  = %lld\n", (long long)ns_count(ms(25)));
    std::printf("protect_ttft               = %d\n", (int)pg.protect_ttft);
    std::printf("expedite                   = %d\n", (int)pg.expedite);
    std::printf("yield_after_chunk          = %d\n", (int)pg.yield_after_chunk);
    std::printf("recommended action         = %s (%s)\n",
                to_string(pg.recommendation.action), to_string(pg.recommendation.reason));

    std::printf("\n[05_ttft_protection] summary: prefill exceeded ttf target -> protect ttft"
                " (expedite).\n");
    return 0;
}
