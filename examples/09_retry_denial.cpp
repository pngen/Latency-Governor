// 09_retry_denial.cpp
// Govern retry when the retry reserve is exhausted -> prohibited (cancel retry).
//
...
int main() {
    ...
    d.contract.retry_allowance = ms(20);
    d.contract.max_retry_delay = ms(30);
    const AdmitResult a = gov.admit(d);

    // A predicted retry (50ms) that alone exceeds the retry allowance.
    const Duration predicted_retry = ms(50);
    const RetryGovernance rg = gov.govern_retry(a.request_id, predicted_retry);
    std::printf("retry_allowance_ns         = %lld\n", (long long)ns_count(d.contract.retry_allowance));
    std::printf("predicted_retry_ns         = %lld\n", (long long)ns_count(rg.predicted_retry_duration));
    std::printf("prohibited                 = %d\n", (int)rg.prohibited);
    std::printf("fail_fast                  = %d\n", (int)rg.fail_fast);
    std::printf("allowed                    = %d\n", (int)rg.allowed);
    std::printf("immediate                  = %d\n", (int)rg.immediate);
    std::printf("recommended action         = %s (%s)\n",
                to_string(rg.recommendation.action), to_string(rg.recommendation.reason));

    std::printf("\n[09_retry_denial] summary: retry reserve exhausted -> retry prohibited"
                " (cancel retry).\n");
    return 0;
}
